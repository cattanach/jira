...
* IF any user who is a member of jira-access OR jira-access-project-restricted
*               AND does not have a Q2 email
* OR who is not a member of jira-access has a role in any project, 
* notify the admins.
*/
...
final userGroupName2 = 'jira-access-project-restricted'
final adminGroupName = 'jira-app-admins' 
...
def userGroup2 = groupManager.getGroup(userGroupName2)
def users = groupManager.getUsersInGroup(userGroup)
def users2 = groupManager.getUsersInGroup(userGroup2) 
users.addAll(users2)
...
String scriptSummary = "${invalidUsers.size()} exceptions were found where external useres have role-based permissions to internal projects. Please review the output and remove any users necessary."
